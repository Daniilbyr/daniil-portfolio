# Портфолио Даниил Кузовлев

Одностраничник с нуля: только HTML/CSS/JS, без сборщика.

## Локально

Откройте `index.html` в браузере или:

```bash
npx --yes serve .
```

## GitHub Pages

1. Создайте репозиторий, например `daniil-portfolio`.
2. Залейте содержимое этой папки в корень или в `/docs`.
3. Settings → Pages → ветка `main`, папка `/ (root)` или `/docs`.
4. Ссылка будет: `https://<user>.github.io/daniil-portfolio/`

Демо внутри этого же репозитория (подпапки на GitHub Pages):

- `https://<user>.github.io/daniil-portfolio/landing-traffic-demo/`
- `https://<user>.github.io/daniil-portfolio/tilda-style-ig-tg-example/`
- `https://<user>.github.io/daniil-portfolio/apollo-men-landing/` (тёмная тема, каталог)
- `https://<user>.github.io/daniil-portfolio/apollo-premium/` (монохром, карточка товара по новым макетам)

В откликах и резюме используйте основную ссылку на портфолио вместо чужих сайтов.
